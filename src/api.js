const SITE = "https://site.api.espn.com/apis/site/v2/sports/tennis/atp";
const WEB  = "https://site.web.api.espn.com/apis/common/v3/sports/tennis/atp";
...
async function get(url) {
  const res = await fetch(url, { headers: { "user-agent": "tennis-api (github.com/LeSingh1/tennis-api)" } });
  if (!res.ok) throw new Error(`ESPN ${res.status} for ${url}`);
  return res.json();
}

// tennis (ATP) is tennis/atp on ESPN.
