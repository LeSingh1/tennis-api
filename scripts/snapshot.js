...
if (games.length === 0) {
  console.log(`${ymd}: no tennis (ATP) games today; leaving snapshot unchanged`);
  process.exit(0);
}
mkdirSync("data", { recursive: true });
writeFileSync(
  "data/latest.json",
  JSON.stringify({ sport: "tennis", league: "atp", date: ymd, count: games.length, games }, null, 2) + "\n",
);
console.log(`${ymd}: wrote ${games.length} tennis (ATP) games -> data/latest.json`);
